...
def has_critical_patterns(text):
    crit_spam_phrases = [
        ...
        r"(?=.*\bдоход\b)(?=.*\bонлайн\b)",
        # "До 1200$ в неделю","от 400 баксов в неделю","От 1000$ в неделю","от 900 долларов в неделю","от $700 в неделю"
        r'(?i)(?:от|до)\s*(?:\$?\s*\d+(?:[.,]\d+)?|\d+(?:[.,]\d+)?\s*(?:баксов|долларов|\$))\s*в\s*неделю'
        ...
        r"(?=.*\bвыигрыш\b)(?=.*\bказино\b)(?=.*\bTONCOIN\b).*",
		  ...
		  # Recruiting
        r"\bИщу\s+амбициозного\s+человека\b",
        r"\bИщу\s+амбициозных\s+людей\b"
	 ]
