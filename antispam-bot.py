...
async def check_automatically(update: Update, context: CallbackContext):
    ...
    # Ban automatically
    if (len(words) < 500 and is_reply is False and has_hashtags_bool is True) and (("✅✅✅✅" in words or "✅✅✅✅" in words.replace('\U0001F537', '✅') or (crit_tokens_bool is True and user_is_premium is True) or num_mixed > 1 or spam_tokens is not None or emoji_critical_num is True)):
        verdict = f"""
<b>Смешанные слова:</b> {num_mixed}; [ {', '.join(mixed_words)} ]
...
<b>4+ одинаковых эмодзи подряд:</b> {repeated_emojis_bool}
    ...
    # suggestion mode
    if (num_regular > 1 or num_crypto > 0 or num_adult > 0 or num_betting > 0 or num_mixed > 1 or repeated_emojis_bool is True) and (len(words) < 500 and has_hashtags_bool is True):
